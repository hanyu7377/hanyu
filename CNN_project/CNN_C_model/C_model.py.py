...
def initialize_model():
    model = Sequential()
    model.add(layers.Conv2D(16, (3, 3), activation="relu",kernel_regularizer=regularizers.l2(0.01), input_shape=(IM_width , IM_width , 1), padding='same'))
    model.add(layers.MaxPool2D(pool_size=(2, 2)))
    model.add(layers.Conv2D(32, (3, 3), activation="relu", kernel_regularizer=regularizers.l2(0.01),padding='same'))
    model.add(layers.MaxPool2D(pool_size=(2, 2)))
    model.add(layers.Conv2D(64, (3, 3), activation="relu", kernel_regularizer=regularizers.l2(0.01),padding='same'))
    model.add(layers.MaxPool2D(pool_size=(2, 2)))
    #model.add(layers.Conv2D(144, (3, 3), activation="relu", kernel_regularizer=regularizers.l2(0.1),padding='same'))
    #model.add(layers.MaxPool2D(pool_size=(2, 2)))
...
my_model.save('../C_model_result/C_model.h5') 
...
plot_loss_or_accuracy('loss','val_loss' , "blue", "Loss plot (C-model)")
plot_loss_or_accuracy('accuracy','val_accuracy' , "orange", "Accuracy plot (C-model)")
